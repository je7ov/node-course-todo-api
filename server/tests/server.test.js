    ...
    request(app)
      ...
      .expect((res) => {
        expect(res.body.todos.length).toBe(
          seed.todos
            .filter((todo) => todo._creator === seed.users[0]._id)
            .length
        );
      })
      .end(done);
...
describe('DELETE /todos/:id', () => {
  ...
  it('should return 404 if trying to remove different user\'s todo', (done) => {
    const hexId = seed.todos[0]._id.toHexString();
...
describe('PATCH /todos/:id', () => {
  ...
  it('should return 404 if trying to update different user\'s todo', (done) => {
    const id = seed.todos[0]._id.toHexString();
    const text = 'new text';
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        User
          ...
          .then((user) => {
            expect(user.tokens
                .filter((token) => token.token === res.header['x-auth']).length
              )
              .toBe(1);
